"""
    ...
    hi<BLANK>
    where hi is case-insensitive 
    and is not immediately followed by the letter D which is also case-insensitive

    Input Format

    First line contains an integer N, 
    N lines follow each line with a sentence not more than 10 words W each, 
...
def verify(s):
    result = re.match("hi\s[^d]", s, re.IGNORECASE)
    if result: print s

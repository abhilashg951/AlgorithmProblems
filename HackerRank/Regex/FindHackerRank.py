"""
    ...
    and have scraped conversations from popular sites. 
    Each conversation fits in 1 line and there are N such conversations. 
    ...
    First line of the input contains N, N lines follow. 
    From the second line onwards, each line contains a set of words W separated by a single space
...
def find_hackerrank(s):
    results_dict = {'starts':1, 'ends':2, 'both':0, 'neither':-1}
    hackerrank = 'hackerrank'
    starts = s[:len(hackerrank)] == hackerrank
    ends = s[-len(hackerrank):] == hackerrank
    result = results_dict['neither']
    if starts and ends: result = results_dict['both']
    elif starts: result = results_dict['starts']
    elif ends: result = results_dict['ends']
    return result

if __name__ == "__main__":
    num_tests = int(raw_input())
    for _ in xrange(num_tests):
        print find_hackerrank(raw_input())

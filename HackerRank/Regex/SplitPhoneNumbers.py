"""
    ...
    Where the country and local area codes can have 1-3 numerals each 
    and the number section can have 4-10 numerals each.
    ...
    try to solve it using regular expressions and groups in order to gain a hands on practice of the concepts involved.
...
def split_phone_number(s):
    result = re.match("(\d{1,3})[\s-](\d{1,3})[\s-](\d{4,10})", s)
    return [x for x in result.groups()] if result else None

if __name__ == "__main__":
    num_tests = int(raw_input())
    for _ in xrange(num_tests):
        result = split_phone_number(raw_input())
        print "CountryCode=" + result[0] \
            + ",LocalAreaCode=" + result[1] \
            +",Number=" + result[2]

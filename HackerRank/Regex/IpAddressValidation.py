"""
    ...
    IPv6, with 128 bits was develped to permit the expansion of the address space. 
    ...
    This is followed by N lines such that each the text in each line is 
    either an IPv4 address or an IPv6 address, or a chunk of text which does not equal either of these. 
    There will be no extra text or whitespace leading or trailing 
    the IP address in a line (if it is an IP address). 
    The number of characters in each line will not exceed 500.

    Output Format
    N lines.
    The ith output line should equal 
    (a)IPv4 or (b)IPv6 or (c)Neither depending on what you detected the ith input line to be.
...
def valid_ipv4(s):
    ...
    for section in sections:
        if not valid_num(int(section)): valid = False
    return valid

def ip_address_type(s):
    result = "Neither"
    IPv4 = re.match('(\d{1,3}\.){3}\d{1,3}$', s)
    IPv6 = re.match('([0-9a-f]{1,4}\:){7}[0-9a-f]{1,4}$', s)
    if IPv4 and valid_ipv4(IPv4.group()): result = "IPv4"
    elif IPv6: result = "IPv6"
    return result

if __name__ == "__main__":
    num_tests = int(raw_input())
    for _ in xrange(num_tests):
        print ip_address_type(raw_input())

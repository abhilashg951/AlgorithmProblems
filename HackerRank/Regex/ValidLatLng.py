"""
    ...
    This is followed by N lines of text. 
    Each line contains a pair of co-ordinates which possibly indicate the latitude and longitude of a place.
    ...
    Invalid
    Explanation
    The first six pairs are valid because X, Y satisfy the criteria related to formatting issues, 
    and they satisfy the conditions restricting the numerical range(s) of X and Y. 
    The next six pairs are all invalid because: The first four (among this invalid group) 
    do not satisfy the conditions restricting the numerical range(s) of X and Y. (90., 180.) 
    is invalid because of an extra dot (.) after 90 and 180.
...
def valid_num(s, min_val, max_val):
    def in_interval(num, min_val, max_val):
        return min_val <= num <= max_val
    def has_trailing_dot(s):
        return s[-1] == '.'
    def has_redundant_zero(s):
        num = re.match("[-+]?(\d+).", s)
        if num:
            res = num.group(1)
            if len(res) > 1 and res[0] == '0': return True
        return False
    return in_interval(float(s), min_val, max_val) and not has_trailing_dot(s) and not has_redundant_zero(s)

def valid_coordinate(s):
    result = re.match("\(([-+]?\d+.?\d*),\s([-+]?\d+.?\d*)\)", s)
    return "Valid" \
        if len(result.groups()) == 2 and \
            valid_num(result.group(1), MIN_LAT, MAX_LAT) and \
            valid_num(result.group(2), MIN_LNG, MAX_LNG) \
        else "Invalid"
    
if __name__ == "__main__":
    num_tests = int(raw_input())
    for _ in xrange(num_tests):
        print valid_coordinate(raw_input())

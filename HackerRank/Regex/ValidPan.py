"""
    ...
    The first PAN number is valid since the first 5 characters are letters, 
    the next 4 are digits and the last character is an alphabet with all the letters in uppercase.
...
def valid_pan(s):
    return "NO" if re.match("[A-Z]{5}\d{4}[A-Z]$", s) is None else "YES"

if __name__ == "__main__":
    num_tests = int(raw_input())
    for _ in xrange(num_tests):
        print valid_pan(raw_input())

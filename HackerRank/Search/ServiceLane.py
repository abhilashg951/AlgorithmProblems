"""
    ...
    He noticed the check engine light was on and wants to service it immediately to avoid any risks. 
    Luckily, a service lane runs parallel to the highway. 
    The length of the highway and service lane is N units. 
    The service lane constitutes of N segments of unit length, where each segment can have different widths. 
    ...
    also implying the width of the vehicle. 
    ...
    where N is the length of the freeway and T is the number of test cases. 
    The next line has N space separated integers which represents the width array.

    T test cases follow. Each test case containts two integers - i & j, 
    where i is the index of segment through which Calvin enters the service lane 
...
def biggest_type(lane_widths, entry, exit):
    return min(lane_widths[entry:exit+1])

if __name__ == "__main__":
    highway_length, num_tests = map(int, raw_input().split())
    lane_widths = map(int, raw_input().split())
    for _ in xrange(num_tests):
        entry, exit = map(int, raw_input().split())
        print biggest_type(lane_widths, entry, exit)

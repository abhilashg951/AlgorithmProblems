"""
    ...
    Since this leaves an extra cell empty on the right, 
    you can shift everything over until V can be inserted. 
    This will create a duplicate of each value, 
    but when you reach the right spot, you can replace a value with V.
...
def insertion_sort(nums):   
    unsorted_num = nums[-1]
    for j in xrange(len(nums) - 2, -2, -1):
        if j == -1: nums[j+1] = unsorted_num
        else: nums[j+1] = max(nums[j], unsorted_num)
        print_nums(nums)
        if unsorted_num == nums[j+1]: break
...
if __name__ == "__main__":
    ...
    if len(nums) > 1: insertion_sort(nums)
